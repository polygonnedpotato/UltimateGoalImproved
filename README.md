...
Even if you are an advanced Java programmer, it is helpful to start with the [FTC Blocks tutorial](https://github.com/FIRST-Tech-Challenge/FtcRobotController/wiki/Blocks-Tutorial), and then migrate to the [OnBot Java Tool](https://github.com/FIRST-Tech-Challenge/FtcRobotController/wiki/OnBot-Java-Tutorial) or [Android Studio](https://github.com/FIRST-Tech-Challenge/FtcRobotController/wiki/Android-Studio-Tutorial) afterwards.
...
* Android Studio users need to be connected to the internet the first time they build the app (to download needed packages for the build).  Clearing Gradle caches will also require an internet connection for the next build.
...
* The default PIDF values for REV motors have been reverted to the default PID values that were used in the 2018-2019 season
    ...
    * This brings the behaviour of the REV motors in line with the behaviour of all other motors
    * If you prefer the 2019-2020 season's behaviour for REV motors, here are the PIDF values that were in place, so that you can manually set them in your OpModes:  
      <br>
...
* Adds setGain() and getGain() methods to the NormalizedColorSensor interface
    * By setting the gain of a colour sensor, you can adjust for different lighting conditions.
      For example, if you detect lower colour values than expected, you can increase the gain.
    * The gain value is only applied to the argb() and getNormalizedColors() methods, not to the raw colour methods.
...
* Merges SensorREVColorDistance Java sample into SensorColor Java sample, which showcases best practices for all colour sensors
...
* Adds support for Sony PS4 gamepad, and reworks how gamepads work on the Driver Station
    * Removes preference which sets gamepad type based on driver position. Replaced with a menu which allows specifying type for gamepads with unknown VID and PID
	* Attempts to auto-detect gamepad type based on USB VID and PID
	* If gamepad VID and PID is not known, use type specified by the user for that VID and PID
...
* Driver Station will now attempt to automatically recover from a gamepad disconnecting, and re-assign it to the position it was assigned to when it dropped
    ...
    * If two gamepads are assigned and have **different** VID/PID signatures, and BOTH drops: both will be recovered
    * If two gamepads are assigned and have **the same** VID/PID signatures, and only one drops: it will be recovered
    * If two gamepads are assigned and have **the same** VID/PID signatures, and BOTH drops: **neither** will be recovered, because of the ambiguity of the gamepads when they re-appear on the USB bus.
...
* Makes various tweaks to the Self Inspect screen
    ...
    * Hides the aeroplane mode entry, when viewing the report of a Control Hub
...
* Reworks network settings portion of Manage page
    ...
    * WiFi channels are filtered by a band (2.4 vs 5 GHz) and whether they overlap with other channels
...
* Improve I2C read performance
    * Very large improvement on Control Hub; up to ~2x faster with small (e.g. 6 bytes) reads
...
* Fixes a slew of bugs with the Vuforia camera monitor including:
    * Fixes a bug where preview could be displayed with a wonky aspect ratio
    * Fixes a bug where preview could be cut off in landscape
    * Fixes a bug where preview got messed up when rotating the phone
    * Fixes a bug where crosshair could drift off target when using webcams
* Fixes an issue in UVC driver on some devices ([ftc_app 681](https://github.com/ftctechnh/ftc_app/issues/681)) if streaming was started/stopped multiple times in a row
...
* Fixes a bug in UVC driver where often half the frames from the camera would be dropped (e.g. only 15FPS delivered during a streaming session configured for 30FPS).
* Fixes an issue where TensorFlow Object Detection would show results whose confidence was lower than
...
* Fixes an issue where device scans would fail while the Robot was restarting
...
* Fixes webcam-frame-to-bitmap conversion problem: the alpha channel wasn't being initialized, only R, G, & B
...
* Fixes an issue where IMU initialization could prevent Op Modes from stopping
* Fixes an issue where AndroidTextToSpeech.speak() would fail if it was called too early
...
* Adds and improves Expansion Hub-related warnings
    ...
    * Displays a warning when hub loses and regains power during an OpMode run
        * Fixes the hub's LED pattern after this condition
    * Displays a warning when Expansion Hub is not responding to commands
        * Specifies whether the condition is current or occurred temporarily during an OpMode run
...
* Adds bulk read functionality for REV Hubs
  ...
  * The following Hub bulk caching modes are available:
...
* Removes PIDF Annotation values added in Rev 5.3 (to AndyMark, goBILDA and TETRIX motor configurations).
  * The new motor types will still be available but their Default control behaviour will revert to Rev 5.2
...
* Blocks related changes:
   * Fixes a bug with blocks generated code when hardware device name is a java or javascript reserved word.
...
* Prevents extra DS User clicks by filtering based on the current state.
...
* Switches out a startup, connect chimes, and error/warning sounds for Star Wars sounds
...
* OnBotJava
    ...
    * A build can be queued, but not started. The OnBot Java build console will display a warning if this occurs.
 ...
 * Includes a fix to avoid deadlock situation with WatchdogMonitor which could result in USB communication errors.
     - Comm error appeared to require that user disconnect the USB cable and restart the Robot Controller app to recover.
     - robotControllerLog.txt would have error messages that included the words "E RobotCore: lynx xmit lock: #### abandoning lock:"
 * Includes fixing to correctly list the parent module address for a REV Robotics Expansion Hub in a configuration (.xml) file.
     - Bug in versions 4.0 and 4.1 would incorrectly list the address module for a parent REV Robotics device as "1".
 ...
 * Block Development Tool Changes
     ...
     - Change the "Save completed successfully." message to a white colour so it will contrast with a green background.
 ...
 * Introduce support for Google's TensorFlow Lite technology for object detection for 2018-2019 game.
     ...
     - Example Java and Block op modes are included to show how to determine the relative position of the gold block (left, centre, right).

## Version 4.1 (released on 18.09.24)

Changes include:
 * Fix to prevent a crash when deprecated configuration annotations are used.
...
* Improved Expansion Hub firmware update support in Robot Controller app
    - Changes to make the system more robust during the firmware update process (when performed through the Robot Controller app).
    - User no longer has to disconnect a downstream daisy-chained Expansion Hub when updating an Expansion Hub's firmware.
        + If a user is updating an Expansion Hub's firmware through a USB connection, he/she does not have to disconnect RS485 connection to other Expansion Hubs.
	...
	+ Robot Controller app should be able to install the firmware onto the Hub, even if accidentally was bricked in a previous update attempt.
 ...
 * Resiliency
	...
	+ This action helps the system recover from some ESD-induced disruptions.
    - Various fixes to improve the reliability of FTC software.
 ...
 * Blocks
    ...
    - Added programming blocks to support PIDF (proportional, integral, derivative and feedforward) motor control.
    - Added formatting options (under Telemetry and Miscellaneous categories) so a user can set how many decimal places to display a numerical value.
    - Support to play audio files (which are uploaded through the Blocks web interface) on Driver Station in addition to the Robot Controller.
 ...
 * Modest Improvements in Logging
    - Decrease the frequency of battery checker voltage statements.
	...
	+ If enabled, the user provides a "Match Number" through the Driver Station user interface (top of the screen).
 ...
 * Miscellaneous
    ...
    - When the user is configuring an Expansion Hub, the LED on the Expansion Hub will change blink pattern (purple-cyan)  to indicate which Hub is currently being configured.
...
Known issues:
 * Initial support for UVC compatible cameras
    - UVC cameras seem to draw a significant amount of current from the USB bus.
        + This does not appear to present any problems for the REV Robotics Control Hub.
	+ This does seem to create stability problems when using some cameras with an Android phone-based Robot Controller.
	+ FTC Tech Team is investigating options to mitigate this issue with the phone-based Robot Controllers.
 ...
 * Wireless
    - When the user selects a wireless channel, this channel does not necessarily persist if the phone is power cycled.
 ...
 * Miscellaneous
    - The blink indication feature that shows which Expansion Hub is currently being configured does not work for a newly created configuration file.
        + User has to first save a newly created configuration file and then close and re-edit the file for blink indicator to work.
 ...
 * Fixes an issue where log files would be closed on the pause of the RC or DS, but not re-opened upon resume.
 * Fixes issue with battery handler (voltage) start/stop race.
 * Fixes an issue where Android Studio generated op modes would disappear from the available list in certain situations.
 * Fixes a problem where OnBot Java would not build on REV Robotics Control Hub.
 * Fixes a problem where OnBot Java would not build if the date and time on the Robot Controller device were "rewound" (set to an earlier date/time).
 * Improved error message on OnBot Java that occurs when renaming a file fails.
 * Removed unneeded resources from android.jar binaries used by OnBot Java to reduce the final size of Robot Controller app.
 ...
 * Robot Controller log has been renamed to /sdcard/RobotControllerLog.txt (note that this change was introduced w/ v3.2).
 ...
 * Blocks
    ...
    - Sample op modes can be selected as a template when creating a new op mode.
    - Fixed bug where the blocks would disappear temporarily when a mouse button is held down.
 ...
 * OnBot Java
    - Sometimes when you push the build button to build all op modes, the RC returns an error message that the build failed.  If you press the build button a second time, the build typically succeeds.
...
The OnBot Java Development Tool is an integrated development environment (IDE) that is served up by the Robot Controller.  Op modes are created and edited using a Javascript-enabled browser (Google Chrome is recommended).  Op modes are saved on the Robot Controller Android device directly.
 ...
 * Enhanced web-based development tools
    ...
    - Web-based management interface (where user can change the Robot Controller name and also easily download Robot Controller log file).
    - OnBot Java, Blocks and Management features available from a web-based interface.
...
Known issues:
* Autocomplete function is incomplete and does not support the following (for now):
     - Access via *this* keyword
     - Access via *super* keyword
     - Members of the superclass, not overridden by the class
     - Any methods provided in the current class
     - Inner classes
     - Can't handle casted objects
     - Any objects coming from a parenthetically enclosed expression

## Version 3.10 (built on 17.05.09)

This version of the software provides support for the REV Robotics Expansion Hub.  This version also includes improvements in the USB communication layer to enhance system resiliency.  If you were using a 2.x version of the software previously, updating to version 3.1 requires that you also update your Driver Station software in addition to updating the Robot Controller software.
...
In addition to the release changes listed below (see section labelled "Version 3.00 (built on 17.04.013)"), version 3.00 has the following important changes:
...
3. Version 3.00 software currently does not have a mechanism to disable extra i2c sensors.  We hope to re-introduce this function with a release soon.
 ...
 * Includes Block programming improvements
    ...
    - Blocks execute after the while-opModeIsActive loop (to allow for cleanup before exiting op mode).
 ...
 * Implements support for the REV Robotics Expansion Hub
    ...
    - Detects REV firmware version and records in a log file.
...
## Version 2.61 (released on 16.12.19)
  * Blocks Programming mode changes:
     - Fix to correct the issue when an exception was thrown because an OpticalDistanceSensor object appears twice in the hardware map (the second time as a light sensor).
...
## Version 2.5 (internal release on released on 16.12.13)
  * Blocks Programming mode changes:
     ...
     - Added support for copying blocks in one OpMode and pasting them in another OpMode. The clipboard content is stored on the phone, so the programming mode server must be running.
     - Modified Utilities section of the toolbox.
     - In Programming Mode, display information about active connections.
     - Fixed paste location when workspace has been scrolled.
     - Added blocks support for the android Accelerometer.
     - Fixed issue where Blocks Upload Op Mode truncated name at the first dot.
     - Added blocks support for Android SoundPool.
     - Added type of safety to blocks for Acceleration.
     - Added type of safety to blocks for AdafruitBNO055IMU.Parameters.
     - Added type of safety to blocks for AnalogInput.
     - Added type of safety to blocks for AngularVelocity.
     - Added type of safety to blocks for Color.
     - Added type of safety to blocks for ColorSensor.
     - Added type of safety to blocks for CompassSensor.
     - Added type of safety to blocks for CRServo.
     - Added type of safety to blocks for DigitalChannel.
     - Added type of safety to blocks for ElapsedTime.
     - Added type of safety to blocks for Gamepad.
     - Added type of safety to blocks for GyroSensor.
     - Added type of safety to blocks for IrSeekerSensor.
     - Added type safety to blocks for LED.
     - Added type of safety to blocks for LightSensor.
     - Added type of safety to blocks for LinearOpMode.
     - Added type of safety to blocks for MagneticFlux.
     - Added type of safety to blocks for MatrixF.
     - Added type of safety to blocks for MrI2cCompassSensor.
     - Added type of safety to blocks for MrI2cRangeSensor.
     - Added type of safety to blocks for OpticalDistanceSensor.
     - Added type of safety to blocks for Orientation.
     - Added type safety to blocks for Position.
     - Added type of safety to blocks for Quaternion.
     - Added type of safety to blocks for Servo.
     - Added type of safety to blocks for ServoController.
     - Added type of safety to blocks for Telemetry.
     - Added type of safety to blocks for Temperature.
     - Added type of safety to blocks for TouchSensor.
     - Added type of safety to blocks for UltrasonicSensor.
     - Added type of safety to blocks for VectorF.
     - Added type of safety to blocks for Velocity.
     - Added type of safety to blocks for VoltageSensor.
     - Added type of safety to blocks for VuforiaLocalizer.Parameters.
     - Added type of safety to blocks for VuforiaTrackable.
     - Added type of safety to blocks for VuforiaTrackables.
...
## Version 2.4 (released on 16.11.13)
  ...
  * Blocks Programming mode changes:
     ...
     - When the user downloads Blocks-generated op mode, only the .blk file is downloaded.
     - When the user uploads Blocks-generated op mode (.blk file), Javascript code is auto-generated.
     - Added DbgLog support.
     - Added logging when a blocks file is read/written.
     - Fixed bug to properly render blocks even if missing devices from a configuration file.
     - Added support for additional characters (not just alphanumeric) for the block file names (for download and upload).
     - Added support for OpMode flavour ("Autonomous" or "TeleOp") and group.
  * Changes to Samples to prevent tutorial issues.
  * Incorporated suggested changes from public pull 216 (“Replace .. paths”).
  * Remove Servo Glitches when robot stopped.
  * if the user hits "Cancels" when editing a configuration file, clears the unsaved changes and reverts to original unmodified configuration.
  * Added log info to help diagnose why the Robot Controller app was terminated (for example, by watchdog function).
  * Added ability to transfer log from the controller.
  * Fixed inconsistency for AngularVelocity
  * Limit unbounded growth of data for telemetry.  If the user does not call telemetry.update() for LinearOpMode promptly, data added for telemetry might get lost if the size limit is exceeded.
...
## Version 2.30 (released on 16.10.05)
  * Blockly programming mode:
     - Mechanism added to save Blockly op modes from Programming Mode Server onto the local device
  ...
  * Fix for autonomous 30-second timer bug (where the timer was in effect, even though it appeared to have timed out).
  ...
  * Fix to allow the use of IrSeekerSensorV3.
 ...
 * Blockly enhancements
    ...
    - support for the ultrasonic sensor.
    - support for IrSeeker.
    - support for LED.
    - support for colour sensor.
    - support for CRServo
    - prompt user to configure the robot before using programming mode.
 * Provides the ability to disable audio cues.
 ...
 * Standardized units in analogue input.
 * Cleaned up code for existing analogue sensor classes.
 * setChannelMode and getChannelMode were REMOVED from the DcMotorController class.  This is important - we no longer set the motor modes through the motor controller.
 * setMode and getMode were added to the DcMotor class.
 * ContinuousRotationServo class has been added to the FTC SDK.
 * Range.clip() method has been overloaded so it can support this operation for int, short and byte integers.
 * Some changes have been made (new methods added) on how a user can access items from the hardware map.
 * Users can now set the zero power behaviour for a DC motor so that the motor will brake or float when power is zero.
 ...
 * Android Studio project supports Android Studio 2.1.x and compiles SDK Version 23 (Marshmallow).
 ...
 * For LinearOpMode the user now must for telemetry.update() to update the telemetry data on the driver station.  This update() mechanism ensures that the driver station gets the updated data properly and at the same time.
 ...
 * A change was made to LinearOpMode to allow a given instance to be executed more than once, which is required for the App Inventor.
 ...
 * Changes made to make the FTC SDK synchronous (significant change!)
    ...
    - For legacy module (NXT compatible), the user no longer has to toggle between reading and write modes when reading from or writing to a legacy device.
 ...
 * Changes made to provide auto stop capabilities for op modes.
    ...
    - If an op mode is stopped by the driver station, but a runaway/uninterruptible thread is persisting, the app will log an error message then force itself to crash to stop the runaway thread.
 * Driver Station UI modified to display lowest measured voltage below current-voltage (12V battery).
 * Driver Station UI modified to have a color background for current-voltage (green=good, yellow=caution, red=danger, extremely low voltage).
 ...
 * Improved software so that Robot Controller (RC) is much more resilient and “self-healing” to USB disconnects:
    - If the user attempts to start/restart RC with one or more module missing, it will display a warning but still startup.
    - When running an op mode, if one or more modules gets disconnected, the RC & DS will display warnings, and the robot will keep on working despite the missing module(s).
 ...
 * removed unused import from the driver station
 ...
 * added some log statements for the thread life cycle.
 * moved gamepad reset logic inside of initActiveOpMode() for robustness
 * changes made to mitigate the risk of race conditions on public methods.
 * changes to try and flag when WiFi Direct name contains non-printable characters.
 * fix to correct race condition between .run() and .close() in ReadWriteRunnableStandard.
 * updated FTDI driver
 * made ReadWriteRunnableStanard interface public.
 * Fixed off-by-one errors in Command constructor
 * moved specific hardware implementations into their package.
 * moved specific gamepad implementations to the hardware library.
 * changed LICENSE file to new BSD version.
 * fixed race condition when shutting down Modern Robotics USB devices.
 * methods in the ColorSensor classes have been synchronized.
 * corrected isBusy() status to reflect the end of the motion.
 ...
 * Added Support for Modern Robotics Gyro.
  ...
  - Users can access heading data (about the Z-axis)
  - Users can also access raw gyro data (X, Y, & Z axes).
  - Example MRGyroTest.java op mode included.
 * Improved error messages
  - More descriptive error messages for exceptions in user code.
 * Updated DcMotor API
 * Enable read mode on a new address in setI2cAddress
 ...
 * a Fixed race condition in EventLoopManager.
 * Fix to keep references stable when updating gamepad.
 * For legacy Matrix motor/servo controllers removed the necessity of appending "Motor" and "Servo" to controller names.
 * Updated HT colour sensor driver to use constants from ModernRoboticsUsbLegacyModule class.
 * Updated MR colour sensor driver to use constants from ModernRoboticsUsbDeviceInterfaceModule class.
 * Correctly handle I2C Address change in all colour sensors
 * Updated/cleaned up op modes.
  ...
  - Added HTRGBExample.java op mode (demonstrates how to use HT legacy colour sensor).
 ...
 * Throws an error if Ultrasonic sensor (NXT) is not configured for legacy module port 4 or 5.
 ...
 * The API for the Legacy Module and Core Device Interface Module has been updated.
   - Support for encoders with the Legacy Module is now working.
 * The hardware loop has been updated for better performance.

This is an improved README that has grammatical corrections and is translated to English(UK).